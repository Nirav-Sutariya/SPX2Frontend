...
function NextGamePalnDynamicICLongMatrix({
  ...
  function valueSetup() {
    BEContractTable = premiums.map((premium, index) => {
      ...
      BE1GainTable[index] = (Number(beContract) > 0 ? ((((5 - premium) * 100 * (Number(beContract) + 1)) - ((Number(beContract) + 1) * commission)) - CumulativeLossTable) : 0).toFixed(2);
      BE2GainTable[index] = (Number(beContract) > 0 ? ((((5 - premium) * 100 * (Number(beContract) + 2)) - ((Number(beContract) + 2) * commission)) - CumulativeLossTable) : 0).toFixed(2);
      BE3GainTable[index] = (Number(beContract) > 0 ? ((((5 - premium) * 100 * (Number(beContract) + 3)) - ((Number(beContract) + 3) * commission)) - CumulativeLossTable) : 0).toFixed(2);
      BE4GainTable[index] = (Number(beContract) > 0 ? ((((5 - premium) * 100 * (Number(beContract) + 4)) - ((Number(beContract) + 4) * commission)) - CumulativeLossTable) : 0).toFixed(2);
      BE5GainTable[index] = (Number(beContract) > 0 ? ((((5 - premium) * 100 * (Number(beContract) + 5)) - ((Number(beContract) + 5) * commission)) - CumulativeLossTable) : 0).toFixed(2);
        ...
        <table className="table-auto border-collapse w-full">
          <thead>
            <tr className="bg-background2 text-white text-sm lg:text-base font-semibold">
              ...
              {Credit && <th className="border-x border-borderColor px-2 py-2 min-w-16">Debit</th>}
            ...
            {filteredPremiums.slice(0, showAll ? filteredPremiums.length : 5).map((premium, index) => {
              ...
              return (
                <tr key={index} className="text-sm lg:text-base text-center text-Secondary bg-background6">
                  {Credit && <td className={`border-t border-borderColor px-2 py-2 ${AfterLossTable > newColumnValue ? "bg-background9" : "bg-background10"}`}>${newColumnValue.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>}
...
export default NextGamePalnDynamicICLongMatrix;