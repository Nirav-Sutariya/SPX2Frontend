...
const generatePDF = async (data, name, emailId, subscriptionStartDate) => {

  // If check stripe invoice available 
  if (data?.stripeId) {
    try {
      let response = await axios.post(process.env.REACT_APP_SUBSCRIPTION_URL + process.env.REACT_APP_GET_SUBSCRIPTION_GET_STRIPE_INVOICE, { userId: getUserId(), stripeId: data.stripeId, startDate: subscriptionStartDate }, {
        headers: {
    ...
    let subscriptionName = data.subscriptionName[0].toUpperCase() + data.subscriptionName.slice(1)
    // Table rows - using dynamic data
    doc.text(`${subscriptionName}`, 20, 100);
    doc.text(`Subscription to ${subscriptionName} features`, 60, 100);
    doc.text(`$${data.subscriptionPrice / 100}`, 140, 100);

    // Subtotal, Discount, Tax, Total
    const subtotal = parseFloat(data.subscriptionPrice / 100);
    const discount = parseFloat(data.couponDiscount / 100);
    const taxRate = 0//0.05;
    const tax = subtotal * taxRate;
    const total = subtotal + tax - discount;

    doc.text(`Subtotal: $${subtotal.toFixed(2)}`, 140, 130);
    doc.text(`Discount: - ${(discount ? discount.toFixed(2) : "0.00")}`, 140, 140);
    doc.text(`Tax (0%): $${tax.toFixed(2)}`, 140, 150);
    doc.text(`Total: $${(total ? total.toFixed(2) : "0.00")}`, 140, 160);
...
const SubscriptionHistory = () => {
  ...
  const userSubscriptionCheckID = async (userSubscriptionId, subscriptionStartDate) => {
    try {
